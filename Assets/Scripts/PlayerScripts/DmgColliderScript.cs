...
namespace PlayerScripts
{
    public class DmgColliderScript : MonoBehaviour
    {
        ...
        private void OnTriggerStay2D(Collider2D other) // Enemy Player'a deger ise Player'ın canını götür.
        {
            ...
            var result = __Calculations.CalculationsAboutToEnemy(other); // Enemy'nin sağında mı solunda mı oldugunu hesaplar.
            Vector2 clampedDirectionToEnemy = result.Item1;
            ...
            {
                CollisionIsActive(other, clampedDirectionToEnemy);
            }
        }

        private void CollisionIsActive(Collider2D other, Vector2 clampedDirectionToEnemy) // bu Dash kullanıldıgında dmg olmasın diye yazılmıştır.
        {
            switch (__SkillsScript.isDashAtackUse)
            {
                case false: // Dash kullanılmadıysa Player dmg alabilir.
                    HitToPlayer(other, clampedDirectionToEnemy);
                    break;
                ...
                case true: // eger Dash kullanıldı ise Player bir süre dmg alamaz.
                    if (canUseDashHitTimer > 1f && other.gameObject != null) // Dash Vurmak için bu var __SkillsScript.isDashAtackUse == TRUE oldugunda çalışır
                    {
                        ...
                        StartCoroutine(__DashAttackDetails.DashAtackToEnemy(other, clampedDirectionToEnemy)); // DashAtack çalışsın
                    }
        ...
        void HitToPlayer(Collider2D other, Vector2 clampedDirectionToEnemy) // Player dmg alıyor
        {
            ...
            __PlayerScript.TakeDamage(dmg, clampedDirectionToEnemy);
